    ...
    <div class="row page-titles mx-0 d-flex flex-wrap align-items-center">
        ...
        <div class="col-sm-6 p-md-0 justify-content-sm-end d-flex">
            <div class="breadcrumb">      
                <a href="{{ route('incoming-stock.create') }}" class="btn btn-xs btn-primary"><i class="fa fa-plus me-2"></i>Tambah Stok Masuk</a>
            </div>
        </div>
    </div>
                        ...
                        <table class="datatables display table table-hover my-2">
                            ...
                            <tbody>
                                @foreach ($stocks as $stock)
                                    <tr>
                                        <td>
                                            <a href="{{ route('incoming-stock.show', $stock) }}" class="text-info">{{ $stock->code }}</a>
                                        </td>
                                        <td>{{ $stock->supplier->name }}</td>
                                        <td>{{ $stock->date->format('d M Y') }}</td>
                                        <td>{{ $stock->note }}</td>
                                        <td>
                                            <form action="{{ route('incoming-stock.destroy', $stock) }}" method="POST">
                                                @csrf
                                                @method('DELETE')
